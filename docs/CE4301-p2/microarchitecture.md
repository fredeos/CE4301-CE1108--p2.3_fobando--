...
En esta sección se detalla la organización interna del procesador diseñado para la arquitectura RISC planteada en el ISA. El procesador se basa en una **arquitectura Harvard** con una separación física de las memorias de instrucciones y datos. Para comparar el rendimiento entre un procesador con caché y sin caché se implementan dos procesadores con accesos a memoria con latencia de lectura y escritura. Se implementa memoria caché únicamente para datos en una jerarquía ```L1-D > L2 > Memoria principal```.

---------

### 1. Procesador sin caché
